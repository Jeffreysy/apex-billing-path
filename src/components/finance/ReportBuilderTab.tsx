...
const ReportBuilderTab = () => {
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter(p => {
      ...
      if (collector !== "all" && p.collectorId !== collector) return false;
      ...
      if (delinquencyStatus === "delinquent" && client.daysAging === 0) return false;
      if (delinquencyStatus === "current" && client.daysAging > 0) return false;
      if (agingBucket !== "all") {
  ...
  }, [reportDateRange, paymentType, paymentSource, collector, clientSearch, caseType, contractStatus, delinquencyStatus, agingBucket, minAmount, maxAmount]);
  ...
  const dateLabel = reportDateRange?.from
    ? reportDateRange.to
      ? `${format(reportDateRange.from, "MMM dd")} — ${format(reportDateRange.to, "MMM dd")}`
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-1">
            ...
            <Select value={paymentSource} onValueChange={setPaymentSource}>
              ...
              <SelectContent>
                ...
                <SelectItem value="crm">CRM / Auto-Pay</SelectItem>
              </SelectContent>
            </Select>
          </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          <strong className="text-foreground">{filteredPayments.length}</strong> transactions totaling{" "}
          <strong className="text-foreground">${totalFiltered.toLocaleString()}</strong>
        </p>
      </div>
      ...
      <div className="dashboard-section overflow-x-auto">
            ...
            {filteredPayments.slice(0, 50).map(p => {
              ...
              return (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant={client?.status === "delinquent" ? "destructive" : "outline"} className="text-xs capitalize">
                      {client?.status ?? "—"}
                    </Badge>
                  </TableCell>
                  <TableCell>
                    {client && client.daysAging > 0
                      ? <Badge variant="destructive" className="text-xs">{client.daysAging}d</Badge>
        ...
        {filteredPayments.length > 50 && (
          <p className="mt-2 text-xs text-muted-foreground text-center">Showing first 50 of {filteredPayments.length} results</p>
        )}
      </div>
    </div>
  );
