...
const CollectorDashboard = () => {
  ...
  if (!collector) {
    return <DashboardLayout><p className="text-muted-foreground">Collector not found.</p></DashboardLayout>;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              {collector.name}
              {collector.isLead && <Badge variant="secondary">Lead Collector</Badge>}
            </h1>
              ...
              <form onSubmit={handlePayment} className="space-y-4">
                <div><Label>Client</Label>
                  <Select><SelectTrigger><SelectValue placeholder="Select client" /></SelectTrigger>
                    <SelectContent>{myClients.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
            ...
            {collectors.map((c) => (
              <div key={c.id} className="flex items-center gap-3 rounded-md border p-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{c.callsMade} calls · ${c.totalCollected.toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {allCalls.map((call) => (
              <div key={call.id} className="flex items-start justify-between border-b pb-3 last:border-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{call.collectorName} · {call.date} · {formatDuration(call.duration)}</p>
