...
import { clients, payments, callLogs, collectors, tasks, getWeeklyCollections, getContractAnalytics, getCaseTypeBilling } from "@/data/mockData";
...
const AdminDashboard = () => {
  ...
  const deptData = [
    { name: "Collections", tasks: tasks.filter(t => t.targetDepartment === "collections" && t.status !== "completed").length, color: "hsl(174 60% 40%)" },
    { name: "Legal", tasks: tasks.filter(t => t.targetDepartment === "legal" && t.status !== "completed").length, color: "hsl(220 70% 22%)" },
    { name: "AR", tasks: tasks.filter(t => t.targetDepartment === "ar" && t.status !== "completed").length, color: "hsl(38 92% 50%)" },
    { name: "Admin", tasks: tasks.filter(t => t.targetDepartment === "admin" && t.status !== "completed").length, color: "hsl(152 60% 40%)" },
  ];
  ...
  return (
    <DashboardLayout title="Admin Dashboard">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">High-level overview of all departments, KPIs, and firm-wide activity</p>
      </div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Total AR Outstanding" value={`$${totalAR.toLocaleString()}`} change={-3.2} icon={<DollarSign className="h-5 w-5" />} />
        <StatCard label="Total Collected (90d)" value={`$${totalCollected.toLocaleString()}`} change={12.5} icon={<TrendingUp className="h-5 w-5" />} />
        <StatCard label="Active Clients" value={String(activeClients)} change={5} icon={<Users className="h-5 w-5" />} />
        <StatCard label="Open Tasks" value={String(openTasks)} change={-8} icon={<AlertTriangle className="h-5 w-5" />} />
      </div>

      {/* Department Activity + Client Status */}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { name: "Collections", icon: Phone, path: "/collections", desc: `${collectors.reduce((s, c) => s + c.callsMade, 0)} calls · ${collectors.reduce((s, c) => s + c.paymentsTaken, 0)} payments taken` },
          { name: "Legal", icon: Scale, path: "/legal", desc: `${clients.filter(c => c.caseStage === "litigation").length} in litigation · ${clients.filter(c => c.status === "new").length} new retainers` },
          { name: "AR Oversight", icon: Eye, path: "/ar-oversight", desc: `$${totalAR.toLocaleString()} outstanding · ${delinquent} delinquent` },
          { name: "Reporting", icon: TrendingUp, path: "/reporting", desc: `${contractData.reduce((s, d) => s + d.started, 0)} contracts started this quarter` },
        ].map((dept) => (
          <Card key={dept.name} className="transition-shadow hover:shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-sm">
                <dept.icon className="h-4 w-4 text-secondary" />
                {dept.name}
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-2">
                <Badge variant="outline" className="text-[10px]">
                  {deptData.find(d => d.name === dept.name || (dept.name === "Reporting" && d.name === "AR"))?.tasks || 0} open tasks
                </Badge>
              </div>
            </CardContent>
          </Card>
