  ...
  return (
    <DashboardLayout title="Collections Department">
      <div className="mb-6 flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Collections Dashboard</h1><p className="text-muted-foreground">Team-wide collection activity and client queue</p></div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Promise to Pay" value={String(promiseToPay)} icon={<Clock className="h-5 w-5" />} />
        <StatCard label="Delinquent Accounts" value={String(delinquent)} icon={<Users className="h-5 w-5" />} />
      </div>
          ...
          <div className="space-y-3">
            {recentCalls.map(call => (
              <div key={call.id} className="flex items-start justify-between border-b pb-3 last:border-0">
                <div><p className="text-sm font-medium">{call.clientName}</p><p className="text-xs text-muted-foreground">{call.collectorName} · {call.date} · {Math.floor(call.duration / 60)}m</p>{call.notes && <p className="mt-1 text-xs text-muted-foreground">{call.notes}</p>}</div>
