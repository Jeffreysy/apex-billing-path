-- HubSpot match strategy pivot
-- Discovery showed HubSpot doesn't back-populate MyCase IDs onto deals (one-way connector).
-- Replacing match_hubspot_deals() to score on dealname + contact_phone + contact_email instead.
-- Same 2-of-3 secure-match principle: score >= 2 = matched, = 1 = review, = 0 = unmatched.
...
BEGIN
  ...
  candidates AS (
    SELECT
      s.hubspot_deal_id,
      c.id AS client_id,
      (CASE WHEN s.norm_email IS NOT NULL AND lower(c.email) = s.norm_email THEN 1 ELSE 0 END) AS s_email,
      (CASE WHEN s.norm_phone10 IS NOT NULL
  ...
  scored AS (
    SELECT
      ...
      (s_email + s_phone + CASE WHEN name_sim >= p_name_threshold THEN 1 ELSE 0 END) AS score
    FROM candidates
  ),
  best AS (
  ...
  SELECT
    ...
    END AS new_status,
    CASE
      WHEN b.s_email = 1 AND b.s_phone = 1 AND b.name_sim >= p_name_threshold THEN 'all_three'
