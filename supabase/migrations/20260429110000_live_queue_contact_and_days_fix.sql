-- Backfill client_id on activities where name matches a client
update public.collection_activities ca
set client_id = c.id
from public.clients c
where ca.client_id is null
  and ca.client_name != ''
  and lower(trim(c.name)) = lower(trim(ca.client_name));

-- Rebuild collections_dashboard with:
--   a) Dynamic days-past-due computed from next_due_date vs current_date
--   b) Name-based fallback in activity_summary when client_id is null
...
latest_case as (
  ...
  order by immigration_cases.client_id,
           immigration_cases.is_closed,
           immigration_cases.open_date desc nulls last
),
-- Resolve every activity to a client_id: first by direct FK, then by name match
resolved_activities as (
  select
    coalesce(ca.client_id, c_name.id) as client_id,
    ca.activity_date,
    ca.outcome
  from public.collection_activities ca
  left join public.clients c_name
    on ca.client_id is null
    and ca.client_name != ''
    and lower(trim(c_name.name)) = lower(trim(ca.client_name))
  where coalesce(ca.client_id, c_name.id) is not null
),
activity_summary as (
  select
    ra.client_id,
    max(ra.activity_date)                                                           as last_contact_date,
    count(*) filter (where ra.activity_date >= current_date - interval '30 days')   as contact_count_30d,
    count(*) filter (
      where ra.activity_date >= current_date - interval '90 days'
        and coalesce(ra.outcome, '') in (
          'payment_taken','promise_to_pay','callback_scheduled','client_satisfied'
        )
    )                                                                               as positive_contact_count_90d
  from resolved_activities ra
  group by ra.client_id
),
...
select
  ...
  co.collected,
  coalesce(co.value, 0) - coalesce(co.collected, 0)    as balance_remaining,
  co.monthly_installment,
  co.next_due_date,
  -- Live days-past-due: max of stored field, stored days_out, and live computation from next_due_date
  greatest(
    coalesce(c.days_past_due, 0),
    coalesce(co.days_out, 0),
    case
      when co.next_due_date is not null
        and co.next_due_date < current_date
        and coalesce(co.value, 0) - coalesce(co.collected, 0) > 0
      then (current_date - co.next_due_date)::int
      else 0
    end
  )                                                     as effective_days_past_due,
  co.days_out,
  co.case_number,
  co.collector,
  ic.lead_attorney,
  ...
  (
    coalesce(co.value, 0) - coalesce(co.collected, 0) > 0
    and coalesce(co.value, 0) - coalesce(co.collected, 0) < settings.low_balance_threshold
  )                                                     as low_balance_hold,
  case
    when commitment.id is not null
      ...
      and commitment.promised_date < current_date - make_interval(days => settings.promise_grace_days)
      then 'broken_promise'
    when coalesce(co.value, 0) - coalesce(co.collected, 0) >= settings.high_balance_threshold
      and greatest(
        coalesce(c.days_past_due, 0), coalesce(co.days_out, 0),
        case when co.next_due_date < current_date and coalesce(co.value, 0) - coalesce(co.collected, 0) > 0
          then (current_date - co.next_due_date)::int else 0 end
      ) > 0
      then 'high_balance'
  ...
  case
    ...
    when coalesce(co.value, 0) - coalesce(co.collected, 0) > 0
      and coalesce(co.value, 0) - coalesce(co.collected, 0) < settings.low_balance_threshold
  ...
  (
    ...
    + least(12, floor((coalesce(co.value, 0) - coalesce(co.collected, 0)) / 1000)::int)
...
where c.is_active = true
  ...
  and (co.id is null or co.status = any (array['Active'::text, 'Risk'::text]))
  and (
    coalesce(co.value, 0) - coalesce(co.collected, 0) > 0
...
order by priority_score desc,
         effective_days_past_due desc nulls last,
         (coalesce(co.value, 0) - coalesce(co.collected, 0)) desc nulls last;
